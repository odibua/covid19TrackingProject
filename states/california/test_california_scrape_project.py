...
# --------------------------
from managers import regression_manager, scrape_manager, training_data_manager, metadata_manager, add_commit_and_push, case_parser_manager, death_parser_manager
...
class TestCaliforniaScrapeAndProject(unittest.TestCase):
    ...
    def test_metadata_manager(self):
        if self.metadata_bool:
            if len(self.state_arg) == 0 or self.state_arg.lower() == self.state_name.lower():
                metadata_manager(state_name=self.state_name)
    ...
    def test_case_training_data_manager(self):
        if self.train_data_bool:
            if len(self.state_arg) == 0 or self.state_arg.lower() == self.state_name.lower():
                training_data_manager(state_name=self.state_name, type='cases')

    def test_death_training_data_manager(self):
        if self.train_data_bool:
            if len(self.state_arg) == 0 or self.state_arg.lower() == self.state_name.lower():
                training_data_manager(state_name=self.state_name, type='deaths')

    def test_regression_case_manager(self):
        if self.regression_bool:
            if len(self.state_arg) == 0 or self.state_arg.lower() == self.state_name.lower():
                regression_manager(state_name=self.state_name, type='cases', regression_type=self.regression_type)
