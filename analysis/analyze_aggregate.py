...
# --------------------------
from visualization import aggregated_vis
...
def main():
    ...
    state_path = os.path.join('state', state, 'csvs')
    ...
    for csv_file in os.listdir(state_path):
        ...
        if 'case' in csv_file.lower():
            csv_path_dict['case'].extend(os.path.join(state_path, csv_file))
        elif 'death' in csv_file.lower():
            csv_path_dict['death'].extend(os.path.join(state_path, csv_file))
        else:
    ...
    series_analysis(csv_df_dict=csv_df_dict)
